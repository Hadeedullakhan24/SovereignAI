...
# ── 6. Mandatory Prompt Examples & Route Disambiguation Suite ──────────────────

class TestMandatoryDisambiguationExamples:
    """Verify all explicit examples from SovereignAI agent routing requirements."""

    @pytest.mark.parametrize(
        "query",
        [
            "generate an image of an industrial heat engine",
            "create an image of a pump",
            "draw a picture of a refinery",
            "generate a diagram of a heat exchanger",
            "create a visualization of a distillation tower",
        ],
    )
    def test_image_generation_mandatory_examples(self, router, query):
        from agent.router import TaskType
        decision = router.route(query)
        assert decision.capability == Capability.IMAGE_GENERATION, f"Failed for {query}"
        assert decision.task_type == TaskType.IMAGE_GENERATION
        assert decision.tool_name == "image_generator"
        assert decision.use_rag_context is False
        assert decision.is_actionable() is True
    ...
    def test_vision_ocr_mandatory_examples(self, router, query):
        from agent.router import TaskType
        decision = router.route(query)
        assert decision.capability == Capability.VISION, f"Failed for {query}"
        assert decision.task_type == TaskType.VISION_OCR
        assert decision.tool_name == "vision_inspector"
        assert decision.use_rag_context is False
    ...
    @pytest.mark.parametrize(
        ...
        [
            "analyze this P&ID",
            "inspect this engineering drawing",
            "analyze this process flow diagram",
            "read this schematic diagram",
            "inspect this blueprint",
        ],
    )
    def test_engineering_drawing_mandatory_examples(self, router, query):
        from agent.router import TaskType
        decision = router.route(query)
        assert decision.capability == Capability.VISION, f"Failed for {query}"
        assert decision.task_type == TaskType.ENGINEERING_DRAWING_ANALYSIS
        assert decision.tool_name == "vision_inspector"
        assert decision.model_record is not None
        assert "Qwen2.5-VL" in decision.model_record.hf_repo_id


# ── 7. Verification of All 12 Public Routes & Authoritative Models ────────────

class TestAllRoutesPreservation:
    """Verify preservation and authoritative model selection for all 12 TaskTypes."""

    @pytest.mark.parametrize(
        "query, expected_task_type, expected_capability, expected_tool, expected_model_substring",
        [
            ("What is the definition of thermodynamics?", "general_qa", Capability.RAG, "rag_pipeline", "SmolLM2"),
            ("What is the design operating pressure of centrifugal pump P-203?", "document_qa", Capability.RAG, "rag_pipeline", "Qwen2.5-1.5B"),
            ("Extract all equipment tags and design parameters from datasheet", "extraction", Capability.RAG, "rag_pipeline", "Qwen2.5-1.5B"),
            ("Compare centrifugal pump vs positive displacement pump", "comparison", Capability.RAG, "rag_pipeline", "Phi-3.5-mini"),
            ("Troubleshoot high vibration and abnormal noise in compressor K-101", "reasoning", Capability.RAG, "rag_pipeline", "Phi-3.5-mini"),
            ("Summarize the inspection report for crude distillation unit", "summarization", Capability.RAG, "rag_pipeline", "Phi-3.5-mini"),
            ("Export turnaround inspection report as actual pdf artifact", "document_generation", Capability.DOCUMENT_GENERATION, "pdf_generator", "Phi-3.5-mini"),
            ("Read text from this scanned image of inspection log", "vision_ocr", Capability.VISION, "vision_inspector", None),
            ("Analyze this P&ID drawing for control loops", "engineering_drawing_analysis", Capability.VISION, "vision_inspector", "Qwen2.5-VL"),
            ("Generate an image of a distillation tower at sunset", "image_generation", Capability.IMAGE_GENERATION, "image_generator", "stable-diffusion"),
            ("Calculate the relief valve sizing and MAWP for vessel V-305", "calculation", Capability.CALCULATION, "calculator", "Qwen2.5-1.5B"),
            ("Write a Python script to parse maintenance logs", "coding", Capability.CODING, "code_interpreter", None),
        ],
    )
    def test_all_twelve_routes_and_specialists(
        self,
        router,
        query,
        expected_task_type,
        ...
        decision = router.route(query)
        assert decision.task_type.value == expected_task_type
        assert decision.capability == expected_capability
        assert decision.tool_name == expected_tool
        if expected_model_substring and decision.model_record:
            assert expected_model_substring.lower() in decision.model_record.hf_repo_id.lower()
