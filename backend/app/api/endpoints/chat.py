...
async def message(chat_id:str,payload:MessageCreate,request:Request,user:User=Depends(get_current_user),db:Session=Depends(get_db)):
 chat=owned(db,chat_id,user); incoming=ChatMessage(chat_id=chat.id,role="user",content=payload.content); task=Task(user_id=user.id,task_type="chat",input_data={"message":payload.content}); db.add_all([incoming,task]); db.commit(); await execute_task(db,user,task,request.state.request_id); reply=ChatMessage(chat_id=chat.id,role="assistant",content=task.output_data["result"] if task.output_data else "Unable to process message",model_used=(task.output_data or {}).get("model_used")); db.add(reply); audit(db,user_id=user.id,action="chat_message",status="success",task_id=task.id,request_id=request.state.request_id); db.commit(); db.refresh(reply); return data({"message":{"id":reply.id,"role":reply.role,"content":reply.content,"model_used":reply.model_used},"task_id":task.id})
